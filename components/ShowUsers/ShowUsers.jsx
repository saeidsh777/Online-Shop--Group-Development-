...
import { getUserList, changeUserRole } from '@/services/user';
  ...
  useEffect(() => {
    if (updatedRole) {
      ...
      setUserList(prevList => prevList.map(user =>
        user.phoneNumber === updatedRole.phoneNumber && user.role !== updatedRole.newRole
    ...
    <div className="p-6">
      <div className="overflow-x-auto rounded-lg shadow-lg">
        <table className="table-auto w-full text-left bg-white rounded-lg">
          ...
          <tbody>
            {userList.map((user) => (
              <tr key={user.phoneNumber} className="hover:bg-gray-100 transition duration-300">
                ...
                <td className="border px-6 py-3">
                  {canChangeRole(user.role) ? (
                    ...
                    >
                      <option value="admin" className="text-indigo-600">Admin</option>
                      <option value="user" className="text-indigo-600">User</option>
                    </select>
                  ) : (
                    <select
                      value={user.role}
                ...
                <td className="border px-6 py-3 flex gap-4 items-center">
                  <button
                    className={`bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600 ${selectedUser?.phoneNumber === user.phoneNumber && showProfile ? 'bg-red-500' : ''}`}
                    onClick={() => handleShowProfile(user)}
                  >
                    {selectedUser?.phoneNumber === user.phoneNumber && showProfile ? 'Hide Profile' : 'View Profile'}
                  </button>
                </td>
              </tr>
            ))}
      ...
      {showProfile && selectedUser && (
        <div className="mt-6 bg-gray-200 p-6 rounded-lg shadow-lg">
          <h2 className="text-2xl font-bold mb-4">Profile of {selectedUser.name}</h2>
